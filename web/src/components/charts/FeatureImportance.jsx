...
export function FeatureImportance({ data: featData, topN = 12, height = 320 }) {
  const { features, importances } = useMemo(() => {
    const items = featData?.features ?? []
    const sorted = [...items].sort((a, b) => b.importance - a.importance).slice(0, topN)
    return {
  ...
  const data = useMemo(() => [{
    ...
    orientation: 'h',
    marker: {
      color: importances.map((_, i) => {
        const t = importances.length > 1 ? i / (importances.length - 1) : 0
